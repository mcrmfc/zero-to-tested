...
Then(/^the hit count increments$/) do
  ...
  start_hit_count = JSON.parse(HTTParty.get(url).body)['hits']
  wait_for_assertion("Hit count hasn't incremented, hit count = #{start_hit_count + 1}") do
    expect(JSON.parse(HTTParty.get(url).body)['hits']).to eq start_hit_count + 1
  end
end
